...
* **Search (Pretraživanje):** Polje na vrhu ekrana koje služi za brzi pronalazak zapisa.
...
1. **Koji je glavni cilj korištenja password managera prema Modulu 1?**
    * a) Povećanje brzine interneta.
    * b) Sigurno upravljanje velikim brojem jedinstvenih i kompleksnih lozinki. **(TOČNO)**
    * c) Izbjegavanje plaćanja pretplata na web stranicama.

2. **Što se događa ako napadač provali u bazu podataka web stranice na kojoj koristite istu lozinku kao i na ostalim računima?**
    * a) Bitwarden će automatski obrisati te račune.
    * b) Svi vaši računi s tom lozinkom postaju ranjivi na napad (Credential Stuffing). **(TOČNO)**
    * c) Ništa, jer svaka stranica ima dodatnu zaštitu.

3. **Dopuni rečenicu:**
    Master lozinka je jedina lozinka koju korisnik mora znati napamet jer ona služi za **DEKRIPCIJU / OTKLJUČAVANJE** cijelog trezora.

4. **Što od navedenog NIJE sigurnosni izazov kod ručnog upravljanja lozinkama?**
    * a) Zapisivanje lozinki na papiriće.
    * b) Korištenje jednostavnih pojmova poput "Lozinka123".
    * c) Korištenje biometrije (otisak prsta) za pristup trezoru. **(TOČNO)**

5. **Kako Bitwarden rješava problem "zaboravljanja" lozinki za desetke različitih servisa?**
    * a) Pohranjuje ih u enkriptirani trezor kojem pristupate jednom lozinkom. **(TOČNO)**
    * b) Šalje vam podsjetnik na e-mail svaki put kad se trebate prijaviti.
    * c) Automatski mijenja lozinku na stranici svaki tjedan.

6. **Prilikom izrade Bitwarden računa, koja je uloga "Password Hint" opcije?**
    * a) To je zamjenska lozinka u slučaju da zaboravite glavnu.
    * b) Kratki podsjetnik koji vam može pomoći da se sjetite Master lozinke. **(TOČNO)**
    * c) Lozinka koju šaljete Bitwarden podršci za resetiranje računa.

7. **Što je "Zero Knowledge" arhitektura Bitwardena?**
    * a) Bitwarden ne zna vašu Master lozinku niti može pristupiti vašim podacima. **(TOČNO)**
    * b) Sustav koji zahtijeva nula znanja od strane korisnika.
    * c) Besplatna verzija softvera s nula dodataka.

8. **U Modulu 2, koji gumb u sučelju koristite za ručni unos nove lozinke?**
    * a) Settings
    * b) Generator
    * c) Gumb "+" (Add Item) **(TOČNO)**

9. **Gdje se u sučelju nalaze opcije za promjenu jezika ili omogućavanje dvorazinske autentifikacije (2FA)?**
    * a) My Vault
    * b) Settings (Postavke) **(TOČNO)**
    * c) Generator

10. **Koja je razlika između "Password" i "Passphrase" opcije u Bitwarden generatoru?**
    * a) Nema razlike.
    * b) Password koristi nasumične znakove, a Passphrase nasumične riječi. **(TOČNO)**
    * c) Passphrase je namijenjena isključivo za mobilne aplikacije.

11. **Što postižete opcijom "Avoid Ambiguous Characters" u generatoru?**
    * a) Izbjegavate znakove koji slično izgledaju (npr. 0 i O, l i 1). **(TOČNO)**
    * b) Generator će kreirati samo lozinke s malim slovima.
    * c) Lozinka će biti kraća od 8 znakova.

12. **Može li Bitwarden generator kreirati i korisnička imena (Usernames), a ne samo lozinke?**
    * a) Da, ima opciju za generiranje nasumičnih korisničkih imena ili e-mail aliasa. **(TOČNO)**
    * b) Ne, to se mora unijeti ručno.

13. **Scenarij: Na poslu trebate podijeliti lozinku za zajednički alat s 5 kolega. Što je najsigurnije rješenje?**
    * a) Poslati lozinku svima u zajedničku Slack/WhatsApp grupu.
    * b) Koristiti "Organizaciju" unutar Bitwardena za sigurno dijeljenje. **(TOČNO)**
    * c) Napisati lozinku na ploču u uredu.

14. **Što su "Mape" (Folders) u Bitwarden trezoru?**
    * a) Alati za dijeljenje s klijentima.
    * b) Način osobne organizacije i kategorizacije zapisa (npr. Posao, Privatno). **(TOČNO)**
    * c) Mjesto gdje se spremaju obrisane lozinke.

15. **Koja je funkcija "Bitwarden Send" alata?**
    * a) Slanje novca između korisnika.
    * b) Sigurno slanje tekstualnih informacija ili datoteka trećim osobama putem linka. **(TOČNO)**
    * c) Automatsko slanje lozinke administratoru sustava.

16. **Što se događa s lozinkom u trezoru ako je administrator obriše iz zajedničke Organizacije?**
    * a) Prestaje biti dostupna svim članovima koji su joj imali pristup putem te organizacije. **(TOČNO)**
    * b) Ostaje trajno spremljena u privatnim trezorima svih zaposlenika.
    * c) Automatski se mijenja na web stranici.

17. **(Točno/Netočno) Bitwarden će vas upozoriti ako pokušate unijeti lozinku na stranicu koja je vizualna kopija prave stranice, ali ima krivi URL.**
    * Odgovor: **TOČNO** (Neće ponuditi auto-fill)

18. **Koji format datoteke Bitwarden najčešće koristi za izvoz (export) trezora ako želite napraviti backup?**
    * a) .jpg
    * b) .csv ili .json **(TOČNO)**
    * c) .mp3

19. **Što je "Master Password Re-prompt" opcija na pojedinačnom zapisu?**
    * a) Zahtjev da ponovno unesete Master lozinku prije nego što vidite tu specifičnu lozinku. **(TOČNO)**
    * b) Automatska promjena lozinke svakih 30 dana.
    * c) Slanje obavijesti na mobitel pri svakom pregledu.

20. **Zašto je važno redovito raditi analizu trezora (Vault Health Reports)?**
    * a) Da provjerite koliko ste prostora potrošili.
    * b) Da identificirate slabe, ponovljene ili kompromitirane lozinke. **(TOČNO)**
    * c) Da vidite tko je od kolega najviše puta koristio trezor.
